...
def load_map(mapFilename):
    ...
    g = WeightedDigraph()
    for line in f:
        src, dest, total, outdoor = line.split(' ', 4)
        src = Node(src)
        dest = Node(dest)

        try:
            g.addNode(src)
            print "Insert node %s" % str(src)
        except ValueError:
            print "Duplicate node %s" % str(src)

        try:
            g.addNode(dest)
            print "Insert node %s" % str(dest)
        except ValueError:
            print "Duplicate node %s" % str(dest)

        g.addEdge(WeightedEdge(src, dest, total, outdoor))
...
def get_path_distance(digraph, path):
    ...
    tod = 0
    for i in range(l - 1):
        d, od = digraph.getNodesDistance(path[i], path[i + 1])
        td += d
        tod += od
...
def dfs(digraph, start, end, maxTotalDist, maxDistOutdoors, path=[], collection=[]):
    ...
    if td > maxTotalDist or tod > maxDistOutdoors:
        return None
    ...
    for node in digraph.childrenOf(start):
        if node not in path:
            dfs(digraph, node, end, maxTotalDist, maxDistOutdoors, path, collection)
...
def bruteForceSearch(digraph, start, end, maxTotalDist, maxDistOutdoors):
    ...
    for path in results:
        td, tod = get_path_distance(digraph, path)
        if (shortest == 0 or td < shortest)\
                and (td <= maxTotalDist and tod <= maxDistOutdoors):
            shortest_path = path
            shortest = td
