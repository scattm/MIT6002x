...
class Patient(object):
    ...
    def update(self):
        ...
        for i in range(l):
            # clear
            if newViruses[i].doesClear():
                self.viruses.remove(newViruses[i])
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for drug in activeDrugs:
            isResistAll &= self.isResistantTo(drug)
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        for virus in self.viruses:
            ...
            for drug in drugResist:
                resistAll &= virus.isResistantTo(drug)
            if resistAll:
                resistPop += 1
    ...
    def update(self):
        ...
        for i in range(l):
            try:
                self.viruses.append(self.viruses[i].reproduce(popDensity, self.activeDrugs))
            except NoChildException:
                next
        return self.getTotalPop()
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    noDrugTime = 150
    drugTime = 150
    totalTime = noDrugTime + drugTime
    noOfVirus = [0.0 for step in range(totalTime)]
    noOfGuttagonolResistanceVirus = [0.0 for step in range(totalTime)]

    for trial in range(numTrials):
        viruses = [ResistantVirus(maxBirthProb, clearProb, resistances, mutProb) for i in range(numViruses)]
        patient = TreatedPatient(viruses, maxPop)

        for step in range(noDrugTime):
            noOfVirus[step] += patient.update()
            noOfGuttagonolResistanceVirus[step] += patient.getResistPop(['guttagonol'])

        patient.addPrescription('guttagonol')

        for step in range(drugTime):
            noOfVirus[step + noDrugTime] += patient.update()
            noOfGuttagonolResistanceVirus[step + noDrugTime] += patient.getResistPop(['guttagonol'])

    for step in range(totalTime):
        noOfVirus[step] /= numTrials
        noOfGuttagonolResistanceVirus[step] /= numTrials

    pylab.plot(range(totalTime), noOfVirus)
    pylab.plot(range(totalTime), noOfGuttagonolResistanceVirus)
    pylab.title('ResistantVirus simulation')
    pylab.xlabel('Time step')
    pylab.ylabel('# Viruses')
    pylab.legend(['All Virus',
                  'Guttagonol Resistance'])
    pylab.show()
